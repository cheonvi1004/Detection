    ...
    def evaluate(self, resource_id: str) -> DomainResult:
        ...
        for original_id, s_data in data.items():
            ...
            if el_type == settings.STRUCTURE_CRACK_TYPE:
                log.debug(f"[{resource_id}] {sname}({sid}) - 균열폭: {current_val}mm (임계 L1:{cfg.crack_l1_threshold}, L2:{cfg.crack_l2_threshold}, L3:{cfg.crack_l3_threshold} 이상)")
            ...
            # --- [진동 판단 로직] ---
                ...
                # 💡 센서 데이터(mm/s)를 cm/sec 단위로 변환하고 소수점 2자리에서 반올림
                val_cm_sec = round(current_val / 10.0, 2)
                
                log.debug(f"[{resource_id}] {sname}({sid}) - 진동가속도: {val_cm_sec}cm/sec (임계 L1:{cfg.vib_l1_threshold}, L2:{cfg.vib_l2_threshold}, L3:{cfg.vib_l3_threshold} 이상)")

                if val_cm_sec >= cfg.vib_l3_threshold:
                    level = AlertLevel.LEVEL_3
                    detail = f"[구조물 이상] {sname}({sid}) 진동가속도 {val_cm_sec}cm/sec (설정치 {cfg.vib_l3_threshold}cm/sec 이상)"
                elif val_cm_sec >= cfg.vib_l2_threshold:
                    level = AlertLevel.LEVEL_2
                    detail = f"[구조물 이상] {sname}({sid}) 진동가속도 {val_cm_sec}cm/sec (설정치 {cfg.vib_l2_threshold}cm/sec 이상)"
                elif val_cm_sec >= cfg.vib_l1_threshold:
                    level = AlertLevel.LEVEL_1
                    detail = f"[구조물 이상] {sname}({sid}) 진동가속도 {val_cm_sec}cm/sec (설정치 {cfg.vib_l1_threshold}cm/sec 이상)"
