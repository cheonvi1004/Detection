...
class CondensationEngine(BaseDetectionEngine):
    ...
    def _evaluate_flowchart2(t_ext: float, rh: float, delta_t: float, gid: str,ext_t_id: str,humid_id: str,wall_id: str, cfg: CondensationConfig) -> tuple[AlertLevel, str,str]:
        ...
        # 💡 [추가] 결로 엔진 상세 평가 로그 (데이터 vs 임계치)
        w_name = cfg.sensor_info_map.get(wall_id, {}).get("sname", wall_id)
        h_name = cfg.sensor_info_map.get(humid_id, {}).get("sname", humid_id)
        t_name = cfg.sensor_info_map.get(ext_t_id, {}).get("sname", ext_t_id)

        resource_id = getattr(cfg, "resource_id", "UNKNOWN")

        log.debug(
            f"[{resource_id}][{gid}] {w_name} / {h_name} / {t_name} 조합 - "
            f"외기온도: {t_ext:.1f}℃ (임계 L1:{cfg.ext_temp_l1_threshold}) | "
            f"외부습도: {rh:.1f}% (임계 L1:{cfg.ext_humid_l1_threshold}, L2:{cfg.ext_humid_l2_threshold}) | "
            f"ΔT(벽체온도-노점): {delta_t:.2f}℃ (임계 L2:{cfg.level2_delta_t}, L3:0.0 이하)"
        )
