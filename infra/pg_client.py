...
class PgRepo:
    ...
    def connect(self) -> None:
        ...
        self._conn = psycopg2.connect(
            ...
            # 💡 옵션에 띄어쓰기와 등호(=)를 있는 그대로 적어줍니다!
            options=f"-c search_path={settings.pg.schema}"
        )
    ...
    def get_condensation_config(self, resource_id: str) -> Optional[CondensationConfig]:
        with self._cur() as c:
            ...
            c.execute("""
                WITH centers AS (
                    SELECT 
                        a.sensor_id, a.sensor_rl_id,
                        CAST(TRIM(a.point_x) AS numeric) AS center_lon,
                        CAST(TRIM(a.point_y) AS numeric) AS center_lat,
                        'GROUP_' || LPAD(ROW_NUMBER() OVER(ORDER BY sensor_rl_id)::text, 2, '0') AS new_group_id
                    FROM iot_sensor_ms a, iot_sensor_resource_rl b, iot_resource_ms c
                    WHERE a.sensor_id = b.sensor_id 
                      and b.resource_id = c.resource_id 
                      and TRIM(sensor_category) =%s 
                ...
                distances AS (
                    ...
                    FROM iot_sensor_ms s, iot_sensor_resource_rl b, iot_resource_ms d
                    CROSS JOIN centers c
                    WHERE s.sensor_id = b.sensor_id 
                      and b.resource_id = d.resource_id 
                      and TRIM(s.sensor_category) IN (%s,%s)
